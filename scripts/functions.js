...
function setBalanceField() {
    ...
    currency.innerHTML = 'credits'
    balanceField.appendChild(currency)

    let balance = document.createElement('div')
    balance.classList.add('balance')
    balanceField.appendChild(balance)
    balance.innerHTML = localStorage.getItem('balance_joker')
}

function changeBalance(amount) {
    let balance = document.querySelector('.balance')
    localStorage.setItem('balance_joker', Number(localStorage.getItem('balance_joker')) + amount)
    balance.innerHTML = localStorage.getItem('balance_joker')
}
